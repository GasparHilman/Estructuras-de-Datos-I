...
AVL_Nodo *avl_mayor(AVL_Nodo* arbol){
  if(!arbol->der){
    return arbol;
  }
  return avl_mayor(arbol->der);
}


AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato, FuncionComparadora comp, FuncionDestructora destroy){
  if(!raiz)
    return NULL;
  
  if(comp(dato, raiz->dato) < 0){
    raiz->izq = avl_nodo_eliminar(raiz->izq,dato,comp,destroy);
  }else if(comp(dato, raiz->dato) > 0){
    raiz->der = avl_nodo_eliminar(raiz->der,dato,comp,destroy);
  }else{
    if((!raiz->izq) || (!raiz->der)){
      AVL_Nodo *nuevaRaiz = ((!raiz->der)?raiz->izq:raiz->der);
      if (destroy != NULL) {
      destroy(raiz->dato);
      }
      free(raiz);
    ...
    AVL_Nodo *mayorIzq = avl_mayor(raiz->izq);
    void *dato_reemplazo = mayorIzq->dato;
    raiz->izq = avl_nodo_eliminar(raiz->izq,mayorIzq->dato,comp,NULL);

    if (destroy != NULL) {
      destroy(raiz->dato);
    }
    raiz->dato = dato_reemplazo;
